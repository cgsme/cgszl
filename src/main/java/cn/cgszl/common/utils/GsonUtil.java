...
import cn.cgszl.common.Constants;
...
public class GsonUtil {
    ...
    public static <T> T mergeObject(T sourceObject, T destObject) {
        ...
        return GsonUtil.jsonToBean(sourceJson, cls);
    }
    ...
    public static JsonArray readJsonArrayFromRequest(
        ...
        JsonObject json = GsonUtil.readJsonFromRequest(request);
        if (null == json) {
            return null;
        }
    ...
    public static <T> T jsonToDateBean(String sourceJson, Class<T> sourceClass) {
        return jsonToBean(sourceJson, sourceClass, DateUtils.DEFAULT_DATE_FORMAT);
    }
